...
// Monaco Editor (loaded from CDN) - using any type since we load dynamically
...
interface QueryBuilderState {
  ...
  generationError: string | null;  // Errors from AI generation (shown in NL section)
  executionError: string | null;   // Errors from query execution (shown in results)
...
type StateListener = (state: QueryBuilderState) => void;
...
class Store {
  ...
  setState(partial: Partial<QueryBuilderState>): void {
...
class QueryAPI {
  ...
  async executeQuery(sql: string): Promise<QueryResults> {
    const formData = new FormData();
  ...
  private extractError(html: string): string | null {
    // Extract error message from HTML response
    const match = html.match(/<span>([^<]+)<\/span>/);
    return match ? match[1] : null;
  }

  private parseResultsHTML(html: string): QueryResults {
...
class SQLEditor {
  ...
  registerCompletionProvider(schema: TableSchema[]): void {
    if (!monaco) return;
...
class UIRenderer {
  private elements: {
    ...
    nlErrorContainer: HTMLElement;
    loadingIndicator: HTMLElement;
    nlLoadingIndicator: HTMLElement;
  };

  constructor() {
    this.elements = {
      promptInput: document.getElementById('nl-prompt') as HTMLInputElement,
      generateBtn: document.getElementById('generate-sql-btn') as HTMLButtonElement,
      runBtn: document.getElementById('run-query-btn') as HTMLButtonElement,
      copyBtn: document.getElementById('copy-sql-btn') as HTMLButtonElement,
      clearBtn: document.getElementById('clear-sql-btn') as HTMLButtonElement,
      downloadBtn: document.getElementById('download-csv-btn') as HTMLButtonElement,
      resultsContainer: document.getElementById('query-results') as HTMLElement,
      nlErrorContainer: document.getElementById('nl-error') as HTMLElement,
      loadingIndicator: document.getElementById('loading-indicator') as HTMLElement,
      nlLoadingIndicator: document.getElementById('nl-loading') as HTMLElement,
    };
  }

  render(state: QueryBuilderState): void {
  ...
  private renderLoading(state: QueryBuilderState): void {
    ...
    if (indicator) {
      if (state.isExecuting) {
        ...
        if (textEl) textEl.textContent = 'Running query...';
  ...
  private renderGenerationError(state: QueryBuilderState): void {
  ...
  private renderResults(state: QueryBuilderState): void {
  ...
  private renderButtons(state: QueryBuilderState): void {
...
class QueryBuilder {
  ...
  private editor: SQLEditor | null = null;
...
function initQueryBuilder(): void {
